...
Transform::Transform(float angleX, float angleY, float angleZ,
    ...
    : x(posX), y(posY), z(posZ)
{
    float sx = sin(DEG_TO_RAD_MACRO(angleX));
    float sy = sin(DEG_TO_RAD_MACRO(angleY));
    float sz = sin(DEG_TO_RAD_MACRO(angleZ));
    float cx = cos(DEG_TO_RAD_MACRO(angleX));
    float cy = cos(DEG_TO_RAD_MACRO(angleY));
    float cz = cos(DEG_TO_RAD_MACRO(angleZ));
...
point2 Camera::project(const point3 &point, bool clip /* = true */, bool doround /* = true */) const
{
    point2 ret;
...
line2 Camera::project(const line3 &line, bool clip /* = true */, bool doround /* = true */) const
{
    line2 ret;
